...
def trips_and_users(trips,users):
    resultlist1 = []
    resultlist2 = []
    reqlist = trips['request_at'].tolist()
    statuslist = trips['status'].tolist()
    cidlist = trips['client_id'].tolist()
    didlist = trips['driver_id'].tolist()
    uidlist = users['users_id'].tolist()
    bidlist = users['banned'].tolist()
    ridlist = users['role'].tolist()
    length = len(reqlist)
    length2 = len(uidlist)
    i = 0
    while i<length:
        resultlist1.append(reqlist[i])
        xdate = reqlist[i]
        k = i
        cancnt = 0
        datecnt = 0
        st = False
        while k<length and reqlist[k]==xdate:
            datecnt+=1
            j = 0
            st = False
            while j<length2:
                if bidlist[j] == "Yes" and cidlist[k] == uidlist[j]:
                    datecnt-=1
                    break
                j+=1
            if (statuslist[k] == "cancelled_by_driver" or statuslist[k] == "cancelled_by_client") and bidlist[cidlist[k-1]] != "Yes":
                cancnt+=1
            k+=1
        resultlist2.append(round(cancnt/datecnt,2))
        i = k
    result = pd.DataFrame({
    "Day":resultlist1,
